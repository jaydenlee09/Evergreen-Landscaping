    ...
    <section className="py-12 sm:py-16 md:py-24 bg-stone-50 w-full overflow-hidden">
      <div className="container mx-auto px-5 sm:px-6">
        <div className="text-center max-w-4xl mx-auto mb-8 sm:mb-12 md:mb-16">
          <h2 className="font-sans text-2xl sm:text-3xl md:text-5xl font-bold text-forest-800 mb-4 sm:mb-6">
            {title}
          </h2>
          <p className="font-sans text-base sm:text-lg md:text-xl text-stone-600 max-w-3xl mx-auto px-2">
            {description}
          </p>
        </div>
            ...
            {[...Array(4)].map((_, setIndex) => (
              <div key={`set-${setIndex}`} className={styles.marqueeContent}>
                {testimonials.map((testimonial, index) => (
                  <div key={`item-${setIndex}-${index}`} className="flex-shrink-0 w-64 sm:w-72 md:w-80">
